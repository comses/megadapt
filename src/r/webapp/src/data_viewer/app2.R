...
server <- function(input, output) {
    ...
    if(length(input$select_factor1) < 1 || length(input$select_factor2) < 1 || length(input$select_factor3) < 1){
      ...
      if (input$select_graph == 2){
        if (input$select_data_type == 1){
          ...
          plott <-ggplot(subsetdf, aes_string(x= "municipality", y = input$select_factor1, size = "fake_year", fill= "municipality")) +
            geom_point(shape = 21) + geom_text(aes(label=municipality),hjust=0, vjust=0)
        }
        if (input$select_data_type == 2){
          subsetdf = data.for.plot %>%
          ...
          plott <-ggplot(subsetdf, aes_string(x= "geographic_id", y = input$select_factor1, size = "fake_year", fill= "municipality")) +
            geom_point(shape = 21, stroke = 0)
        }
  ...
  output$graphtypeUI <- renderUI({
    selectInput("select_graph", "Plot Type", choices = plotList(),
                width = 150
    )
  })
  ...
  plotList <- reactive({
    plots = list("Scatterplot","Change through time")
    listvalues = (1:2)
    names(listvalues) <- plots
    plots <- as.list(listvalues)
  })
  ...
  datatypeList <- reactive({
    places = list(translator$t("Municipality"), "AGEB")
    listvalues = (1:2)
    names(listvalues) <- places
    as.list(listvalues)
