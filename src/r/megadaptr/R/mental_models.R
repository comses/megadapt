create_mental_models <- function(potable_water_sacmex_limit_strategy,
  ...
  potable_water_sacmex_limit <- get_limit_df(potable_water_sacmex_limit_strategy, year = year, study_data = study_data)
  sewer_water_sacmex_limit <- get_limit_df(sewer_water_sacmex_limit_strategy, year = year, study_data = study_data)
  resident_limit <- get_limit_df(resident_limit_strategy, year = year, study_data = study_data)
...
read_unweighted_matrix <- function(path) {
  ...
  cluster_sizes <- (cluster_name_change_indices - dplyr::lag(cluster_name_change_indices))[-1]
...
create_weighted_matrix <-
  function(unweighted_matrix_meta,
    ...
    list(
      data = t(t(weighted_element_size) / apply(weighted_element_size, 2, sum)),
      labels = unweighted_matrix_meta$labels
    )

  }

create_limit_df <- function(weighted_matrix_meta, tolerance = 1e-10) {
  ...
  dplyr::bind_cols(
    labels = weighted_matrix_meta$labels,
    value = W_matrix_B[,1]
  )
}
...
create_limit_df_from_unweighted_matrix_file <-
  function(path, cluster, tolerance = 1e-8) {
    ...
    weighted_matrix_meta <-
      create_weighted_matrix(
        unweighted_matrix_meta = unweighted_matrix_meta,
        cluster = cluster
      )
...
find_cluster_given_weighted_and_unweighted <- function(weighted_matrix_path, unweighted_matrix_path) {
...
file_mental_model_strategy <- function(paths, limit_df_picker, cluster) {
  limit_dfs <-
    lapply(paths, create_limit_df_from_unweighted_matrix_file, cluster = cluster)
