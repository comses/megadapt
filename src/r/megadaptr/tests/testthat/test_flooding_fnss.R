...
describe('a flooding delta method fnss', {
  ...
  flooding_fnss_val <- call_fnss(flooding_fnss, study_data)

  it('should decrease if capacity increases', {
    study_data$sewer_system_capacity <- 2 * study_data$sewer_system_capacity
    flooding_fnss_high_cap <- call_fnss(flooding_fnss, study_data)
  ...
  it('should increase if precipitation increases', {
    ...
    flooding_fnss_high_precip <- call_fnss(flooding_fnss, study_data)
  ...
  it('should increase if runoff increases', {
    ...
    flooding_fnss_high_runoff <- call_fnss(flooding_fnss, study_data)
